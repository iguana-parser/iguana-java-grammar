...
use java::{
    ...
    parse_tree::{JavaParseTreeBuilder, create_parse_tree, to_json, to_sexpr_with},
    parser::JavaParser,
};
#[cfg(feature = "profile")]
...
#[derive(ClapParser)]
#[command(about = "Parser for the Java grammar")]
...
struct Cli {
    ...
    ext: Option<String>,
    ///Run the Java parser in REPL mode
    ///
    ...
    if args.check_sexpr || args.regenerate_sexpr {
        ...
        let passed = cli::run_golden(
            ...
            |path| {
                ...
                let mut parser = JavaParser::new(&input, start_nonterminal_id, &vec_arena);
        ...
        return cli::run_benchmark(config, move || {
            ...
            for (label, files) in &groups {
                ...
                eprintln!(
                    "{}{} ms",
                    pad,
                    cli::group_digits(&format!("{:.0}", source_time.as_secs_f64() * 1000.0))
                );
            }
            ...
            eprintln!(
                ...
                kind,
                run_num,
                cli::group_digits(&format!("{:.0}", run_ms))
            );
            cli::PhaseTimings {
                input,
                init,
